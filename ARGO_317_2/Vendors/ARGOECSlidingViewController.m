...
/* 为避免名字冲突，名字文件添加“ARGO”起头  */

#import "ARGOECSlidingViewController.h"

NSString *const ARGOECSlidingViewUnderRightWillAppear    = @"ARGOECSlidingViewUnderRightWillAppear";
NSString *const ARGOECSlidingViewUnderLeftWillAppear     = @"ARGOECSlidingViewUnderLeftWillAppear";
NSString *const ARGOECSlidingViewUnderLeftWillDisappear  = @"ARGOECSlidingViewUnderLeftWillDisappear";
NSString *const ARGOECSlidingViewUnderRightWillDisappear = @"ARGOECSlidingViewUnderRightWillDisappear";
NSString *const ARGOECSlidingViewTopDidAnchorLeft        = @"ARGOECSlidingViewTopDidAnchorLeft";
NSString *const ARGOECSlidingViewTopDidAnchorRight       = @"ARGOECSlidingViewTopDidAnchorRight";
NSString *const ARGOECSlidingViewTopWillReset            = @"ARGOECSlidingViewTopWillReset";
NSString *const ARGOECSlidingViewTopDidReset             = @"ARGOECSlidingViewTopDidReset";

@interface ARGOECSlidingViewController()
...
- (ARGOECSlidingViewController *)slidingViewController
{
  UIViewController *viewController = self.parentViewController;
  while (!(viewController == nil || [viewController isKindOfClass:[ARGOECSlidingViewController class]])) {
    viewController = viewController.parentViewController;
  }
  
  return (ARGOECSlidingViewController *)viewController;
}

@end

@implementation ARGOECSlidingViewController
...
- (void)anchorTopViewTo:(ECSide)side animations:(void (^)())animations onComplete:(void (^)())complete
{
  ...
  [UIView animateWithDuration:0.25f animations:^{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      NSString *key = (side == ECLeft) ? ARGOECSlidingViewTopDidAnchorLeft : ARGOECSlidingViewTopDidAnchorRight;
...
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter] postNotificationName:ARGOECSlidingViewTopWillReset object:self userInfo:nil];
  });
...
- (void)topViewHorizontalCenterWillChange:(CGFloat)newHorizontalCenter
{
  CGPoint center = self.topView.center;
	...
	if (center.x >= self.resettedCenter && newHorizontalCenter == self.resettedCenter) {
		dispatch_async(dispatch_get_main_queue(), ^{
			[[NSNotificationCenter defaultCenter] postNotificationName:ARGOECSlidingViewUnderLeftWillDisappear object:self userInfo:nil];
		});
	}
	
	if (center.x <= self.resettedCenter && newHorizontalCenter == self.resettedCenter) {
		dispatch_async(dispatch_get_main_queue(), ^{
			[[NSNotificationCenter defaultCenter] postNotificationName:ARGOECSlidingViewUnderRightWillDisappear object:self userInfo:nil];
		});
	}
...
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter] postNotificationName:ARGOECSlidingViewUnderLeftWillAppear object:self userInfo:nil];
  });
...
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter] postNotificationName:ARGOECSlidingViewUnderRightWillAppear object:self userInfo:nil];
  });
...
{
  dispatch_async(dispatch_get_main_queue(), ^{
    [[NSNotificationCenter defaultCenter] postNotificationName:ARGOECSlidingViewTopDidReset object:self userInfo:nil];
  });
